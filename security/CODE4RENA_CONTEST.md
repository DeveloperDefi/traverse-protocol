# Traverse Protocol — Code4rena Security Contest Brief

## Overview

| Field | Value |
|---|---|
| **Project** | Traverse Protocol (TRV) |
| **Type** | Intent-based cross-chain liquidity router |
| **Prize Pool** | 50,000 TRV (~$50,000 target at launch valuation) |
| **Timeline** | 7 days (exact dates TBD — targeting Q3 2026) |
| **Repo** | https://github.com/Estivalett/traverse-protocol |
...
Prizes are paid in TRV from the Community & Incentives allocation (350M TRV pool).  
...
|---|---|---|
| `contracts/TRV.sol` | ~60 | ERC-20 token — 1B fixed supply, Permit, Votes |
| `contracts/TraverseIntent.sol` | ~140 | EIP-712 intent struct, storage, signing |
| `contracts/TraverseRouter.sol` | ~260 | Core router — submit, fill, cancel, expire intents |
| `contracts/TraverseStaking.sol` | ~230 | Solver staking, revenue distribution, slashing |
| `contracts/TraverseGovernor.sol` | ~70 | On-chain governance (Governor + Timelock) |
| `contracts/TraverseTimelock.sol` | ~30 | 48-hour governance timelock |
| `contracts/TraverseTreasury.sol` | ~120 | Protocol treasury |
| `contracts/TraverseVesting.sol` | ~200 | Team/investor vesting (cliff + linear) |
...
Traverse Protocol is an intent-based cross-chain liquidity router with competitive solver auctions.
...
3. Registered solvers (TRV-staked) call `fillIntent()` — first valid solver wins
...
- Solvers must maintain ≥ 10,000 TRV stake to remain registered
...
4. **TRV used as reward token** — Using the staking token as reward token is not explicitly prevented. Check for potential accounting issues.
...
- **Vesting bypass** — early claim vectors in TraverseVesting
...
- **Cooldown bypass** — ways to extract staked TRV before the 7-day cooldown
